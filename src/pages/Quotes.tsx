...
import {
  ...
  useToast } from
'@chakra-ui/react';
import { DownloadIcon, FileTextIcon, HistoryIcon, MailIcon, PlusIcon, PrinterIcon, Trash2Icon } from 'lucide-react';
...
export function Quotes() {
  ...
  const [form, setForm] = useState({ number: '', person_id: '', amount: 0, tax: 0, status: 'Draft', items: 1, notes: '' });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ number: nextNumber(), person_id: '', amount: 0, tax: 0, status: 'Draft', items: 1, notes: '' });
    formDrawer.onOpen();
  };

  const openEdit = (quote: Quote) => {
    setEditing(quote);
    setForm({ number: quote.number, person_id: quote.person_id ?? '', amount: quote.amount, tax: quote.tax, status: quote.status, items: quote.items, notes: quote.notes });
    formDrawer.onOpen();
  };
  ...
  const handleSubmit = async () => {
    ...
    if (editing) {
      const { error } = await supabase.from('quotes').update({
        ...
        tax: Number(form.tax), status: form.status, items: Number(form.items), notes: form.notes
      }).eq('id', editing.id).eq('user_id', session!.user.id);
      if (!error) toast({ title: 'Quote updated', status: 'success', duration: 2000, position: 'top-right' });
    } else {
      const { error } = await supabase.from('quotes').insert({
        user_id: session!.user.id, number: form.number, person_id: form.person_id || null,
        amount: Number(form.amount), tax: Number(form.tax), status: form.status, version: 1,
        items: Number(form.items), notes: form.notes
      });
  ...
  const handleExport = () => {
    exportToCsv('quotes.csv', quotes.map((q) => {
      ...
      return { number: q.number, customer: p?.company ?? '', amount: q.amount, tax: q.tax, status: q.status, version: q.version, items: q.items, created: q.created_at };
    }));
    ...
    <>
      ...
      <Card>
        ...
        {loading ? (
          ...
          <EmptyState icon={FileTextIcon} title="No quotes found" description="Create a new quote to get started." action={<Button size="sm" bg="navy.600" color="white" borderRadius="9px" fontSize="12px" leftIcon={<PlusIcon size={15} />} onClick={openCreate}>New quote</Button>} />
        ) : (
          <TableContainer>
            <Table size="sm">
              <Thead>
      ...
      <FormDrawer isOpen={formDrawer.isOpen} onClose={formDrawer.onClose} title={editing ? 'Edit quote' : 'New quote'} subtitle={editing ? 'Update quote details' : 'Create a new quotation'} loading={saving} onSubmit={handleSubmit} submitLabel={editing ? 'Update' : 'Create'}>
        <FormControl>
